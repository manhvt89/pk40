...
|
*/
$config['rest_default_format'] = 'json'; // Định dạng dữ liệu mặc định (json|xml|csv)
...
|
*/
$config['enable_emulate_request'] = true; // Cho phép giả lập các phương thức HTTP thông qua tham số `method`
...
|
*/
$config['rest_auth'] = 'bearer';// Kiểu xác thực (none|basic|digest|session|bearer)
...
|
*/
$config['auth_source'] = 'custom';// Nguồn xác thực (ldap|database)
...
|
*/
$config['rest_valid_logins'] = ['manhvt' => 'A991997@123#@!'];
...
|
*/
$config['rest_keys_table'] = 'ospos_keys';
...
|
*/
$config['rest_enable_keys'] = FALSE; // Kích hoạt xác thực API Key
...
|
*/
$config['rest_key_column'] = 'key'; // Tên cột trong csdl sẽ chứa API Key
...
|
*/
$config['rest_key_length'] = 32; // Độ dài của API Key
...
|
*/
$config['rest_key_name'] = 'X-API-KEY'; // Tên trường header sẽ chứa API Key
...
$config['rest_access_token_column'] = 'access_token'; // Tên cột access_token trong bảng database
$config['rest_access_token_life'] = 3600; // Thời gian sống của access_token (theo giây)
$config['rest_access_token_life_refresh'] = TRUE; // Làm mới thời gian sống của access_token mỗi lần xác thực thành công

$config['enable_emulate_request'] = TRUE; // Cho phép giả lập các phương thức HTTP thông qua tham số `method`
$config['rest_limit_method'] = 'METHOD_NAME'; // Giới hạn truy cập vào API dựa trên phương thức HTTP
$config['rest_limit_table'] = 'api_limits'; // Tên bảng chứa dữ liệu giới hạn truy cập API
$config['rest_limit_enable'] = TRUE; // Bật hoặc tắt giới hạn truy cập API